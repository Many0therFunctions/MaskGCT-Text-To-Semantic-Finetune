...
class FactorizedVectorQuantize(nn.Module):
    ...
    def forward(self, z):
        ...
        if self.training:
            ...
            commit_loss = (F.mse_loss(z_e, z_q.detach(), reduction="mean") * self.commitment)
            #codebook_loss = (F.mse_loss(z_q, z_e.detach(), reduction="none").mean([1, 2]) * self.codebook_loss_weight)
            codebook_loss = (F.mse_loss(z_q, z_e.detach(), reduction="mean") * self.codebook_loss_weight)
        else:
            commit_loss = torch.zeros(z.shape[0], device=z.device)
            codebook_loss = torch.zeros(z.shape[0], device=z.device)

        # used to be z_q = z_e + (z_q - z_e).detach()
        z_q = z_e + (z_q - z_e).detach()

        #added detact() to the end
        z_q = self.out_project(z_q).detach()

        # Oh I'm trippin, why are we even returning z_e like that??? It's not even used anywhere right now...
        #return z_q, commit_loss, codebook_loss, indices, z_e
        return z_q, commit_loss, codebook_loss, indices
