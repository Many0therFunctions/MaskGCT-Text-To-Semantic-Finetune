...
from maskgct.g2p.g2p.chinese_model_g2p import BertPolyPredict
from maskgct.g2p.utils.front_utils import *
import os
...
word_pinyin_dict = {}
with open(
    r"./maskgct/g2p/sources/chinese_lexicon.txt", "r", encoding="utf-8"
) as fread:
...
with open(
    r"./maskgct/g2p/sources/pinyin_2_bpmf.txt", "r", encoding="utf-8"
) as fread:
    txt_list = fread.readlines()
    for txt in txt_list:
...
with open(
    r"./maskgct/g2p/sources/bpmf_2_pinyin.txt", "r", encoding="utf-8"
) as fread:
    txt_list = fread.readlines()
    for txt in txt_list:
        v, k = txt.strip().split("\t")
